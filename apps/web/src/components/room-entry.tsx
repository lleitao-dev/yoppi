...
export function RoomEntry({ gameType, title, description }: RoomEntryProps) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-16">
      ...
      <p className="mt-12 text-sm font-semibold uppercase tracking-[0.24em] text-emerald-400">
        Yoppi
      </p>
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            You become the host and receive a six-character room code.
          </p>
          <button
            disabled={busy}
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            If play is already underway, you will wait for the game's next safe admission point.
          </p>
          <label className="mt-5 block text-sm text-slate-300" htmlFor="roomCode">
            Room code
          </label>
          <input
            id="roomCode"
