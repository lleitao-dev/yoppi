...
function resultLabel(
  result: BlackjackStateView['players'][number]['result'],
  net: number,
): string | null {
...
export function BlackjackTable({ room, playerId, state, socket, onError }: BlackjackTableProps) {
  ...
  async function command(
    ...
    const result = await new Promise<CommandAck>((resolve) =>
      socket.emit(event, { roomId: room.id }, resolve),
    );
    ...
    <section className="mt-8 overflow-hidden rounded-3xl border border-emerald-300/15 bg-emerald-950/40 shadow-2xl">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="text-right text-sm text-slate-300">
            ...
            <p className="text-slate-500">
              Bet {state.minBet}–{state.maxBet} · 3:2 blackjack
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-8 sm:px-8">
        <div className="mx-auto max-w-xl text-center">
          ...
          {state.dealer.total !== null && (
            <p className="mt-3 text-sm font-medium text-slate-300">Total {state.dealer.total}</p>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                {resultLabel(player.result, player.net) && (
                  <p
                    className="mt-3 font-semibold text-emerald-300"
                    data-testid={`blackjack-result-${player.playerId}`}
                  >
                    {resultLabel(player.result, player.net)}
                  </p>
                )}
          ...
          {can('BET') && you ? (
            ...
            <div className="flex flex-wrap gap-3">
              {can('HIT') && (
                <button
                  disabled={pending}
                  onClick={() => void command('blackjack:hit')}
              ...
              {can('STAND') && (
                <button
                  disabled={pending}
                  onClick={() => void command('blackjack:stand')}
              ...
              {can('DOUBLE') && (
                <button
                  disabled={pending}
                  onClick={() => void command('blackjack:double')}
                  className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-5 py-3 font-semibold text-amber-200 disabled:opacity-50"
                >
                  Double
                </button>
              )}
              {can('NEXT_ROUND') && (
                <button
                  disabled={pending}
                  onClick={() => void command('blackjack:nextRound')}
                  className="rounded-xl bg-emerald-400 px-5 py-3 font-semibold text-emerald-950 disabled:opacity-50"
                >
                  Next round
                </button>
              ...
              {state.allowedActions.length === 0 && (
                <p className="py-2 text-sm text-slate-400">
                  {state.phase === 'BETTING'
                    ? 'Waiting for the other players to bet.'
