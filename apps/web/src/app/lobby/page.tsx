  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-6 py-16">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-emerald-400">Yoppi Lobby</p>
        ...
        <div className="text-right text-sm text-slate-400">
          ...
          <button className="mt-1 hover:text-white" onClick={() => void leaveSession()}>End guest session</button>
        </div>
      </div>

      <div className="mt-12 grid gap-6 md:grid-cols-2">
        <Link href="/games/blackjack" className="group rounded-2xl border border-white/10 bg-white/5 p-7 hover:border-emerald-400/40 hover:bg-white/[0.07]">
          ...
          <p className="mt-3 leading-7 text-slate-400">Create a private table or join with a room code.</p>
          <p className="mt-8 font-medium text-white">Open Blackjack →</p>
        </Link>

        <Link href="/games/poker" className="group rounded-2xl border border-white/10 bg-white/5 p-7 hover:border-emerald-400/40 hover:bg-white/[0.07]">
