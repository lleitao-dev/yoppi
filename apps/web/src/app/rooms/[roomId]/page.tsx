...
export default function WaitingRoomPage() {
  ...
  const connectedCount = useMemo(
    () => room?.players.filter((player) => player.connected).length ?? 0,
    [room],
  );
  const seatedCount = useMemo(
    () =>
      room?.players.filter((player) => player.participation === 'PLAYING' && player.seat !== null)
        .length ?? 0,
    [room],
  );
  const queuedCount = useMemo(
    () => room?.players.filter((player) => player.participation === 'QUEUED').length ?? 0,
    [room],
  );
  ...
  const eligibleCount = useMemo(
    () =>
      room?.players.filter((player) => player.connected && player.participation === 'WAITING')
        .length ?? 0,
    [room],
  );
  ...
  async function startGame() {
    ...
    const result = await new Promise<CommandAck>((resolve) =>
      socket.emit('game:start', { roomId }, resolve),
    );
      ...
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-3 text-slate-400">
            ...
            {room.status === 'ACTIVE'
              ? `${seatedCount}/${room.maxPlayers} seated${queuedCount > 0 ? ` · ${queuedCount} queued` : ''}`
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 px-6 py-4 text-center">
          ...
          <p
            ...
            className="mt-1 font-mono text-2xl font-semibold tracking-[0.18em]"
          >
            {room.code}
          </p>
        </div>
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {player.seat === null ? 'Waiting for a seat' : `Seat ${player.seat + 1}`} ·{' '}
                      {player.participation.toLowerCase()}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-slate-400">
                    <span
                      className={`h-2.5 w-2.5 rounded-full ${player.connected ? 'bg-emerald-400' : 'bg-slate-600'}`}
                    />
            ...
            {room.gameType === 'BLACKJACK' ? (
              <>
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  Any connected room member can start once the game minimum is met. Blackjack
                  requires {room.minPlayers} player and supports up to {room.maxPlayers}.
                </p>
                <p className="mt-2 text-sm text-slate-500">
                  {eligibleCount} connected waiting player{eligibleCount === 1 ? '' : 's'} currently
                  eligible to start.
                </p>
                <button
                ...
                >
                  {starting
                    ...
                    : room.canStart
                      ? 'Start Blackjack'
                      : `Need ${room.minPlayers} connected player${room.minPlayers === 1 ? '' : 's'}`}
                </button>
              </>
            ) : (
              <>
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  Any connected room member can start once the game minimum is met. Texas
                  Hold&apos;em requires {room.minPlayers} players and supports up to{' '}
                  {room.maxPlayers}. Blinds are 10/20 with 1,000 starting chips.
                </p>
                <p className="mt-2 text-sm text-slate-500">
                  {eligibleCount} connected waiting player{eligibleCount === 1 ? '' : 's'} currently
                  eligible to start.
                ...
                >
                  {starting
                    ...
                    : room.canStart
                      ? "Start Texas Hold'em"
                      : `Need ${room.minPlayers} connected players`}
                </button>
              </>
            )}
          </section>
        ...
        <section className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">
                Host:{' '}
                {room.players.find((player) => player.playerId === room.hostPlayerId)
                  ?.displayName ?? 'Unassigned'}
              </p>
            </div>
            <p className="text-xs text-slate-500">
              Available {room.playerRequirement.current}/{room.playerRequirement.minimum} · maximum{' '}
              {room.maxPlayers}
            </p>
          </div>
          <div className="mt-4 flex flex-wrap gap-2">
            {room.players.map((player) => (
              <span
                key={player.playerId}
                className="rounded-full border border-white/10 bg-black/15 px-3 py-1.5 text-xs text-slate-300"
              >
                {player.displayName}
                {player.isHost ? ' · host' : ''} · {player.participation.toLowerCase()} ·{' '}
          ...
          {membership?.participation === 'QUEUED' && (
            <div
              data-testid="queued-player-notice"
              className="mt-5 rounded-xl border border-sky-300/20 bg-sky-300/10 px-4 py-3 text-sm text-sky-100"
            >
              You are waiting to join the next {room.gameType === 'POKER' ? 'hand' : 'round'}. You
              may watch the current table while your seat is pending.
            </div>
          )}
          {membership?.participation === 'LEAVING' && (
            <div className="mt-5 rounded-xl border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-sm text-amber-100">
              Your departure is pending and will be finalized at the end of the current{' '}
              {room.gameType === 'POKER' ? 'hand' : 'round'}.
            </div>
          )}
          <button
            disabled={!socket?.connected || leaving}
          ...
          >
            {leaving
              ? 'Leaving…'
              : membership?.participation === 'PLAYING'
                ? `Leave after this ${room.gameType === 'POKER' ? 'hand' : 'round'}`
                : 'Leave room'}
          </button>
        </section>
      )}
